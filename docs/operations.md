...
**注意(`ALLOW_CUSTOM_HOMESERVERS=true` 運用時)**: この deactivate API は自サーバー(このホームサーバー)が管理するアカウントにしか効きません。`ALLOW_CUSTOM_HOMESERVERS=true` でログインしている他ホームサーバー(matrix.org 等)のユーザーのアカウント自体を無効化する権限はこちらにはありません。そうしたユーザーの荒らし対応は 2.2 のルームブロック/purge、または当該ルームでの kick/ban(2.3)でカバーしてください。
...
`--drill` は定期的に実行し、「バックアップから実際に復元できる」ことを継続的に確認してください。目安として**月次**で 1 回、cron や手元のリマインダーで回してください(依存パッケージの更新や `homeserver.yaml` のスキーマ変更で、復元手順が気づかないうちに陳腐化することがあります)。

### 6.1 自動バックアップ(systemd timer)の前提: sudo の NOPASSWD

`scripts/systemd/selfmatrix-backup.{service,timer}` は非 root ユーザーで `backup.sh` を実行しますが、`backup.sh` の中身は署名鍵・`homeserver.yaml`・メディアの読み取りに `sudo` を使います。systemd サービスには対話用の TTY が無いため、対象ユーザーに **NOPASSWD の sudoers エントリを事前設定しない限り、sudo は毎回失敗してタイマー実行のたびにバックアップが失敗します**。

`visudo` で次のような行を追加してください(`REPLACE_WITH_DEPLOY_USER` は unit ファイル内の値と合わせる):

```
REPLACE_WITH_DEPLOY_USER ALL=(root) NOPASSWD: /usr/bin/tar, /usr/bin/find, /usr/bin/chown
```

(実際に呼ばれるコマンドは `scripts/backup.sh` を参照し、必要な範囲に絞ってください。`ALL` で丸ごと NOPASSWD にするのは避けてください。)

**失敗に気付く手段**: サイレントに失敗し続けるのを防ぐため、次のいずれかを設定してください。

- 手動確認: `systemctl status selfmatrix-backup.service`(`journalctl -u selfmatrix-backup.service` で過去ログ)
- 自動通知: unit の `[Unit]` セクションに `OnFailure=` を追加し、失敗時に通知用の別 unit(メール送信・Webhook 送信など)を起動する

```
[Unit]
OnFailure=selfmatrix-backup-failure-notify.service
```

`selfmatrix-backup-failure-notify.service` 側の実装(メール/Slack/Discord webhook 等)は運用者の環境に合わせて用意してください。
...
**推奨: 保管先を暗号化する。** `scripts/backup.sh` の出力(`homeserver.yaml`、Synapse 署名鍵、PostgreSQL ダンプ)は平文の秘密情報です。素の `rclone sync` はオブジェクトストレージ側の権限にしか守られないため、次のいずれかで保管先自体を暗号化することを推奨します。

- [`rclone crypt`](https://rclone.org/crypt/) — 既存の `rclone` remote の上に暗号化レイヤーを重ねる方式。追加ツール不要で移行しやすい。
- [restic](https://restic.net/) / [borg](https://www.borgbackup.org/) — バックアップ専用ツールで、クライアント側暗号化・重複排除・世代管理をまとめて扱える。
- [age](https://github.com/FiloSottile/age) — `scripts/backup.sh` が作った `backups/<timestamp>/` の tar/dump を送信前に `age` で個別に暗号化する軽量な方式。

いずれの方式でも、復号鍵/パスフレーズ自体は `backups/` や VPS 上に保存しないでください(バックアップの意味が失われます)。
...
- **RAM**: hub のオーディオスタックは接続人数に応じて増えます (実測: 2 クライアントで ≈400MB、workspace の hires-spike.md)。1GB クラスの VPS では使うときだけ起動し、常用するなら 2GB クラスへの増強を前提にしてください。
